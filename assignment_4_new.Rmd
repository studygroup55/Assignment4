...
for (i in 1:n_agents) {
  d <- simulated_responses %>% subset(c == "1.2" & w == "optimal" & agent == 1)
...
draws_df_0.2 <- as_draws_df(samples_agent1_0.2_optimal$draws())
...
draws_df_0.2 <- draws_df_0.2 %>% 
  rename (w_1 = "w[1]",
         w_prior_1 = "w_prior[1]")


draws_df_0.7 <- draws_df_0.7 %>% 
  rename (w_1 = "w[1]",
         w_prior_1 = "w_prior[1]")
...
ggplot(draws_df_0.7) +
  ...
  geom_density(aes(w_1), fill = "red", alpha = 0.3) +
  #geom_vline(xintercept = 0.2, linetype = "dashed", color = "black", size = 1.5) +
  xlab("Weight 2") +
  ylab("Posterior Density") +
  theme_classic()
