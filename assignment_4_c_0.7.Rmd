...
for (i in 1:n_agents) {
  d <- simulated_responses %>% subset(c == "0.7" & w == "optimal" & agent == 1)
...
```{r}
#Extract samples
samples_agent1_0.7_optimal <- samples

#Extract draws
draws_df_0.7 <- as_draws_df(samples_agent1_0.7_optimal$draws())


# Checking the model's chains (noise)
ggplot(draws_df_0.7, aes(.iteration,  c, group = .chain, color = .chain)) +
  geom_line() +
  theme_classic()
...
#draws_df_0.7 <- draws_df_0.7 %>% rename (w_1 = "w[1]", w_prior_1 = "w_prior[1]")

#draws_df_0.7 <- draws_df_0.7 %>% rename (w_2 = "w[2]",w_prior_2 = "w_prior[2]")
